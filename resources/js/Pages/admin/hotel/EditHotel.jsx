...
const EditHotel = ({ id, hotel }) => {
  const [name, setName] = useState(hotel.name);
  const [lokasi, setLokasi] = useState(hotel.lokasi);
  const [kontak, setKontak] = useState(hotel.kontak);
  const [instagram, setInstagram] = useState(hotel.instagram);
  const [deskripsi, setDeskripsi] = useState(hotel.deskripsi);
  const [harga, setHarga] = useState(hotel.harga);
  ...
  const saveData = (e) => {
    ...
    Inertia.post(`/admin/hotel/${id}`, formData, {
  ...
  return (
    <div className="px-4 md:px-12 max-w-5xl mx-auto bg-white rounded-lg pt-6 md:pt-12 mt-6">
      <h3 className="text-2xl font-semibold mb-4">Form Edit Hotel</h3>
      <hr className="mb-4" />
      <Link as="button" type="button" href="/admin/hotel" className="bg-blue-500 text-white px-3 py-1 rounded mb-4 hover:bg-blue-700">
        <h2>Kembali</h2>
      </Link>
